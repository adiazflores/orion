...
FINAL_TASK=$(echo $NEW_TASK_DEF | jq '.taskDefinition|{family: .family, volumes: .volumes, executionRoleArn: "arn:aws:iam::166954342823:role/or-dev-graphql-exec", taskRoleArn: .taskRoleArn, containerDefinitions: .containerDefinitions, networkMode: .networkMode, cpu: .cpu, memory: .memory, requiresCompatibilities: .requiresCompatibilities}')
